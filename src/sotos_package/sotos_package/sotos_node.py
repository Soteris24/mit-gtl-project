...
class EncoderAvoidanceNode(Node):
    ...
    def control_loop(self):
        ...
        if self.state == State.NORMAL:
            ...
            if self.has_arrived():
                ...
                self.target_left = self.left_ticks + self.DRIVE_TICKS
                self.target_right = self.right_ticks + self.DRIVE_TICKS
            ...
            if self.has_arrived():
                ...
                self.target_left = self.left_ticks + self.TURN_TICKS 
    ...
    def check_range(self, msg):
        ...
        is_obstacle_near = 0.01 < msg.range < 0.20

        if (self.state == State.NORMAL or self.state == State.AVOID_DRIVE_PAST) and is_obstacle_near:
            
            if self.state == State.AVOID_DRIVE_PAST:
                self.get_logger().warn("Still too close.. Turning out further...") #debuge
            else:
            ...
            self.target_left = self.left_ticks
            self.target_right = self.right_ticks + self.TURN_TICKS
    ...
    def drive_wheels(self):
        ...
        cmd_l = max(min(err_l * kp, self.MAX_SPEED), -self.MAX_SPEED)
        cmd_r = max(min(err_r * kp, self.MAX_SPEED), -self.MAX_SPEED)
